...
async def main(test_config, init_id, process_engine_setting, storage_engine_setting):
    ...
    for k, v in config_setting.items():
        ...
        if v_range:  # discrete ranged parameter
            ...
            space[k] = (v['min'], v['max'])
    optimizer.bo.space = space

    valid_num = 0
    # 读取上一段运行的结果
    for i in range(0, init_id + 1):
        ...
        exist_gp_config_path = result_dir / f'{i}_gp_config.yml'
        # 读取配置
        ...
        exist_gp_config = yaml.load(
            exist_gp_config_path.read_text()
        )

        exist_config = {
            **(exist_process_engine_config if test_config.hosts.processEngine.tune else {}),
            **(exist_storage_engine_config if test_config.hosts.storageEngine.tune else {}),
        }
        ...
        if i != 0:  # not adding default info, 'cause default cannot convert to numeric form
            ...
            optimizer.add_observation(
                ((exist_config, exist_gp_config), exist_result)
            )
    ...
    while valid_num < test_config.optimizer.iter_limit:
        ...
        gp_config_path = result_dir / f'{task_id}_gp_config.yml'
        gp_config_path.write_text(
            yaml.dump(sampled_config_numeric, default_flow_style=False)
        )

        _print(f'{task_id}:  process_engine_config & storage_engine_config & gp_config generated.')
